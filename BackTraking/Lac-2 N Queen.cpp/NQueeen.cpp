...
public:
    bool isSafe(int row, int col, vector<string>& board, int n){
        // Check for row(only left side), diagonals(up and down), col(no need)
        //For row
        int x = row;
        int y = col;
        while(y >= 0){
...
public:
    ...
    void solve(int colNo, vector<string>& board, vector<vector<string>>& ans, int n, vector<int>& leftRow, vector<int>& upperDiagonal, vector<int>& lowerDiagonal){
        //Base Case: You have placed all your n queens
        if(colNo == n) {
            ans.push_back(board);
            return;
    ...
    vector<vector<string>> solveNQueens(int n) {
        ...
        vector<int> upperDiagonal(2*n -1, 0); // for 4 sized board indexes can be 0 1 2 3 4 5 6 so there are 7 elements means 2*4 -1 elements that's why size is 2*n-1
